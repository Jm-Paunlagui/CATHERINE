 ...
 *
 * Renders the seven permission toggles for T_EMP_MGMT_ADMIN rows.
    ...
    return (
        <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
                ...
                {description && (
                    <p className="text-xs text-black/45 dark:text-white/40 mt-0.5 leading-relaxed">{description}</p>
                )}
            </div>
            <Toggle
                checked={checked}
 ...
 * @returns {JSX.Element}
 */
export function PermissionFlagsFieldset({
    form,
    onToggle,
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-blue-400/8 dark:bg-blue-400/15 border border-blue-400/25">
                ...
                <p className="text-xs text-black/60 dark:text-white/60 font-aumovio">
                    These flags control UI visibility only. The backend re-reads from the database on every action — stale tokens cannot bypass server-side gates.
                </p>
            </div>
            ...
            <PermissionToggleRow
                ...
                description="Inactive admins cannot log in and are blocked from token refresh."
            ...
            {/* Reset request flags */}
            <p className="text-xs font-aumovio-bold text-black/50 dark:text-white/40 uppercase tracking-wide">
                Wallet Reset Requests
            </p>
            <PermissionToggleRow
                label="Can Approve Resets"
                description="Approve pending excess-fund reset requests."
                checked={form.canApproveReset === "Y"}
                onChange={() => onToggle("canApproveReset")}
                disabled={disabled}
            />
            <PermissionToggleRow
                label="Can Reject Resets"
                description="Reject pending excess-fund reset requests."
                checked={form.canRejectReset === "Y"}
                onChange={() => onToggle("canRejectReset")}
                disabled={disabled}
            />
            ...
            <PermissionToggleRow
                label="Can Approve Billing Downloads"
                description="Approve re-download requests in the Billing section."
                checked={form.canApproveBilling === "Y"}
                onChange={() => onToggle("canApproveBilling")}
                disabled={disabled}
            />
            <PermissionToggleRow
                label="Can Reject Billing Downloads"
                description="Reject re-download requests in the Billing section."
                checked={form.canRejectBilling === "Y"}
                onChange={() => onToggle("canRejectBilling")}
                disabled={disabled}
            />
            <PermissionToggleRow
                label="Receive Auto-Billing Emails"
                description="Opted-in admins receive the monthly billing workbook by email."
                checked={form.canReceiveBilling === "Y"}
                onChange={() => onToggle("canReceiveBilling")}
                color="purple"
                disabled={disabled}
            />
            <PermissionToggleRow
                label="Can Export Billing Report"
                description="Download the billing Excel workbook directly from the Billing page."
                checked={form.canExportBilling === "Y"}
                onChange={() => onToggle("canExportBilling")}
                disabled={disabled}
            />
