...
// ─── Hermetic crypto config ───────────────────────────────────────────────────
// This harness asserts argon2-specific behaviour (e.g. a "$argon2id$" prefix and
// the argon2 tuning snapshot). Argon2Adapter is a thin ALIAS to the mode-driven
// CryptoVault — Argon2Adapter.hashPassword() emits whatever PASSWORD_HASH_MODE
// resolves to. The deployment .env may run tripledes/bcrypt (HRIS-interop), under
// which these argon2 assertions would (correctly) fail. So we pin argon2 mode and
// a valid pepper BEFORE loading .env and requiring CryptoVault. dotenv does not
// override already-set process.env vars, so these win over the .env values; the
// CryptoVault singletons read mode/pepper lazily on first use, after this point.
process.env.PASSWORD_HASH_MODE = "argon2";
process.env.PASSWORD_ENCRYPTION_MODE = "argon2";
if (
    !process.env.ARGON2_PEPPER ||
    process.env.ARGON2_PEPPER.trim().length < 32
) {
    process.env.ARGON2_PEPPER = crypto.randomBytes(32).toString("hex");
}
// Pin tuning to the documented defaults so the config-snapshot assertions are
// deterministic regardless of any .env overrides.
process.env.ARGON2_MEMORY_COST = "19456";
process.env.ARGON2_TIME_COST = "2";
process.env.ARGON2_PARALLELISM = "1";
process.env.ARGON2_HASH_LENGTH = "32";
...
async function it(label, fn) {
    try {
        ...
        // Surface the actual failure reason, not just the error's constructor name —
        // a reporter that hides the message makes failures undiagnosable.
        const detail = err?.message || err?.constructor?.name || "Error";
