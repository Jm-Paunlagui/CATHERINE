...
describe("POST /api/v1/auth/logout — integration", function () {
    ...
    describe("happy path — authenticated user", function () {
        ...
        it("clears the access-token cookie (Max-Age=0 or past Expires)", async function () {
            ...
            const cleared = cookies.find((c) => c.includes("app.access-token"));
            expect(
                cleared,
                "access-token clear instruction not found in Set-Cookie",
            ).toBeDefined();

            const isCleared =
                /Max-Age=0/i.test(cleared) ||
                /Expires=Thu, 01 Jan 1970/i.test(cleared) ||
                (/Expires=([^;]+)/.test(cleared) &&
                    new Date(cleared.match(/Expires=([^;]+)/)?.[1]) <
                        new Date());
            expect(
                isCleared,
                "cookie was not cleared (Max-Age or Expires not set to past)",
            ).toBe(true);
        });
...
describe("GET /api/v1/auth/me — integration", function () {
    ...
    it("authenticated → 200 with full JWT payload in data", async function () {
        ...
        expect(res.status).toBe(HTTP_STATUS.OK);
        expect(res.body.status).toBe("success");
        expect(res.body.data.userId).toBe("10001");
        expect(res.body.data.role).toBe("ADMIN");
        expect(res.body.data.GID).toBe(20001);
    });
