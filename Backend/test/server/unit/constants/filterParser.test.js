...
describe("filterParser — parseFilter", function () {
  ...
  describe("$in / $nin", function () {
    ...
    it("$in with exactly 1000 values stays a single IN group", function () {
      ...
      expect(whereClause.match(/ IN \(/g)).to.have.length(1);
      expect(whereClause).to.not.include(" OR ");
      expect(Object.keys(binds)).to.have.length(1000);
    });
    ...
    it("$in with >1000 values splits into OR-joined IN groups (ORA-01795)", function () {
      ...
      expect(whereClause.match(/ IN \(/g)).to.have.length(3);
      expect(whereClause.match(/ OR /g)).to.have.length(2);
      expect(Object.keys(binds)).to.have.length(2500);
      // Every value still bound — no raw interpolation in the IN lists
      expect(Object.values(binds)).to.include(0);
      expect(Object.values(binds)).to.include(2499);
    });
    ...
    it("$nin with >1000 values splits into AND-joined NOT IN groups", function () {
      ...
      expect(whereClause.match(/NOT IN \(/g)).to.have.length(2);
      expect(whereClause.match(/ AND /g)).to.have.length(1);
      expect(Object.keys(binds)).to.have.length(1500);
    });
  });
