...
function makeStore() {
    return new CacheStore(`lockout-test-${++_seq}`, {
        ttl: 86400,
        checkPeriod: 3600,
    });
}
...
describe("LoginLockoutMiddleware — unit tests", function () {
    ...
    describe("check()", function () {
        it("unknown userId → { locked: false }", function () {
            const l = makeLockout();
            expect(l.check("unknown-user-xyz")).toEqual({
                locked: false,
            });
        });
        ...
        it("HR-reset state (all cycles exhausted) → { locked: false, hrReset: true }", async function () {
            ...
            expect(result.hrReset).toBe(true);
            expect(result.locked).toBe(false);
        });
    ...
    describe("recordFailure()", function () {
        it("increments failCount by 1 on each call", function () {
            ...
            expect(l._readState("user-F").failCount).toBe(1);
            l.recordFailure("user-F");
            expect(l._readState("user-F").failCount).toBe(2);
        });
        ...
        it("increments cycles counter when lockout engages", function () {
            ...
            expect(l._readState("user-H").cycles).toBe(1);
        });
        ...
        it("resets failCount to 0 after lockout engages", function () {
            ...
            expect(l._readState("user-I").failCount).toBe(0);
        });
        ...
        it("lockout duration increases each cycle in incremental mode", async function () {
            ...
            expect(Math.abs(dur1 - BASE)).toBeLessThan(15);
            ...
            expect(Math.abs(dur2 - BASE * 2)).toBeLessThan(20);
        });
        ...
        it("lockout duration is constant across cycles in fixed mode", async function () {
            ...
            expect(Math.abs(dur1 - FIXED)).toBeLessThan(15);
            expect(Math.abs(dur2 - FIXED)).toBeLessThan(15);
        });
        ...
        it("currentMax decrements by retryDecrement after each lockout cycle", async function () {
            ...
            l.recordFailure("user-L");
            l.recordFailure("user-L");
            expect(l._readState("user-L").currentMax).toBe(2);
            ...
            l.recordFailure("user-L");
            l.recordFailure("user-L");
            expect(l._readState("user-L").cycles).toBe(2);
            expect(l._readState("user-L").currentMax).toBe(1);
        });
        ...
        it("currentMax never drops below 1 (max(1, ...) floor enforced)", async function () {
            ...
            l.check("user-M");
            l.recordFailure("user-M");
            expect(l._readState("user-M").currentMax).toBeGreaterThanOrEqual(
                1,
            );
        });
    });
    ...
    describe("recordSuccess()", function () {
        it("deletes userId state — _readState returns factory defaults", function () {
            ...
            expect(state.lockUntil).toBe(null);
        });
        ...
        it("subsequent check() after recordSuccess → { locked: false }", function () {
            ...
            expect(l.check("user-O").locked).toBe(false);
        });

        it("recordSuccess on an unknown userId does not throw", function () {
            const l = makeLockout();
            expect(() => l.recordSuccess("never-seen-user")).not.toThrow();
        });
    });
    ...
    describe("full progressive lockout sequence (incremental mode, 3 cycles → HR-reset)", function () {
        it("3 fails → lock → 2 fails → lock → 1 fail → lock → HR-reset", async function () {
            ...
            l.recordFailure(USER);
            l.recordFailure(USER);
            expect(l.check(USER).locked).toBe(true); // cycle 1: should be locked
            ...
            l.recordFailure(USER);
            l.recordFailure(USER);
            expect(l.check(USER).locked).toBe(true); // cycle 2: should be locked
            expect(l._readState(USER).cycles).toBe(2);
            ...
            l.check(USER);
            l.recordFailure(USER);
            expect(l.check(USER).locked).toBe(true); // cycle 3: should be locked
            expect(l._readState(USER).cycles).toBe(3);

            await sleep(100); // 15 * 2^2 = 60ms lock; wait 100ms

            // ── All cycles exhausted → HR-reset ──────────────────────────────────────
            const final = l.check(USER);
            expect(final.hrReset).toBe(true); // should be hrReset after all cycles
            expect(final.locked).toBe(false); // should not be locked in hrReset state
        });
    });
    ...
    describe("user state isolation", function () {
        it("locking user-A does not lock user-B", function () {
            ...
            expect(l.check("iso-user-A").locked).toBe(true);
            expect(l.check("iso-user-B").locked).toBe(false);
        });
        ...
        it("recordSuccess on user-A does not clear user-B's state", function () {
            ...
            expect(l.check("iso-user-D").locked).toBe(true);
        });
    });
});
