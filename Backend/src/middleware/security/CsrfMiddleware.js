...
const { middlewareMessages } = require("../../constants/messages");
...
class CsrfMiddleware {
  constructor(options = {}) {
    ...
    // Fail-fast in ALL environments — a missing secret is a configuration error
    // regardless of NODE_ENV.  A dev fallback string is intentionally rejected
    // to prevent accidental deployment with a predictable secret.
    if (!this._secret) {
      throw new Error(
    ...
    const { generateCsrfToken, doubleCsrfProtection } = doubleCsrf({
      ...
      getTokenFromRequest: (req) =>
        req.headers["x-csrf-token"] ?? req.body?._csrf ?? req.body?.csrfToken,
    });
  ...
  _refreshHandler(req, res) {
    // AuthMiddleware.authenticate (applied at the route level) always populates
    // req.user before this handler runs. The fallback guards against accidental
    // misconfiguration without masking the real identity when auth is present.
    ...
    try {
      ...
      if (!req.cookies?.[cookieName]) {
        logger.warning(middlewareMessages.CSRF_REFRESH_NO_SESSION(userId), {
          ip: req.ip,
          userId,
          userAgent: req.get("user-agent"),
      ...
      logger.info(middlewareMessages.CSRF_TOKEN_REFRESHED(userId), {
        ip: req.ip,
        userId,
        userAgent: req.get("user-agent"),
        endpoint: req.originalUrl,
      ...
      logger.error(middlewareMessages.CSRF_REFRESH_FAILED(userId), {
        error: error.message ?? String(error),
        stack: error.stack,
        ip: req.ip,
        userId,
